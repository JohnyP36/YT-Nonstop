...
let YTNonstop=function t(e){
  ...
  const n=
    {
      loadedAt:Date.now(),_autoSkip:null,_autoLoop:null,
      _debug:false,
  ...
  function d()
   {
               ...
               if(t.some(t=>t.type==="attributes"))
                 {
                   ...
                   if(t)
                     {
                       c(t);if(n.getDebug()==true)console.log("click")
                     }
                     else
                       {
                         l();
     ...
     setInterval(
       ()=>
         {yt.util&&yt.util.activity&&yt.util.activity.setTimestamp();e.setLoop()},
       5e3);
       return n
   }
