...
n.append(`Autotube = ${t}()`);o.appendChild(n);u.append("autotube = Autotube = new Autotube();");o.appendChild(u);n.remove()}let Autotube=function t(e)
...
const u={player:()=>document.getElementById("movie_player"),loop:{button:()=>a()[1],status:function()
 ...
 function a(){return[...document.getElementsByClassName("header ytd-playlist-panel-renderer")[0].getElementsByClassName("style-scope yt-icon-button")].filter(t=>t.id=="button")}
const l=()=>{if(n.getIsAutoSkip()==true&&u.player().getPlayerState()===0){u.player().setAutonav(true);
const t=u.player().getPlaylistIndex();const e=u.player().getPlaylist();if(e===null||e===undefined)
         {return u.player().nextVideo()}if(n.getIsAutoLoop())
         {const o=()=>{const n=Math.abs(Math.floor(Math.random()*e.length));
                       if(n==t)return o();return n};u.player().playVideoAt(o());return}
...
function d()
 {const t={getButton:window.document.getElementsByClassName("ytp-play-button ytp-button")[0]||window.document.getElementById("play-pause-button"),
           ...
           {const t=window.document.getElementById("confirm-button")||window.document.getElementsByClassName("ytmusic-you-there-renderer")[2]||undefined;
            if(t){c(t);if(n.getDebug()==true)console.log("click")}else{l();i();if(n.getDebug()==true)console.log("no click")}}}};
const e={setInterval:setInterval(()=>{if(window.location.href.indexOf("/watch")==-1)return;try{const n=new o(t.callback);n.observe(t.getButton,t.config);e.setLoop();
...
setInterval(()=>{yt.util&&yt.util.activity&&yt.util.activity.setTimestamp();e.setLoop()},5e3);return n}
 ...
 function S(){return u}function t()
     {this.loadedAt=f;this.isAutoSkip=p;this.isAutoLoop=g;this.getReloadDurationCheck=h;this.setReloadDuration=t=>m(t);this.setDebug=t=>b(t);this.get_yt=S;d()}
const A=(t,e)=>{switch(t){case"autoSkip":n.setAutoSkip(e);break;case"autoLoop":n.setAutoLoop(e);if(e!==u.loop.status())u.loop.button()&&u.loop.button().click();break}};
